...
import common.{ OrderingOpsExpOpt => _, _} // reference to OrderingOpsExpOpt is ambiguous; [error] it is imported twice in the same scope by
...
class TestEffects extends FileDiffSuite {
 ...
 @virtualize trait Impl extends DSL with ArrayMutationExp with PrimitiveOpsExp with OrderingOpsExpOpt
